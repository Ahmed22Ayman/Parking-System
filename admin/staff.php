...
if (isset($_POST['submit'])) {
    ...
    $indexQuery = "SELECT * FROM staff WHERE indexnumber=:indexnumber LIMIT 1";
    ...
    if (count($errors) === 0) {
        ...
        $addSecurityQuery = "INSERT INTO staff(indexnumber, name,faculty, department, password, contact)
                VALUES(:indexnumber, :name, :faculty, :department,:password, :contact);
        ...
        if ($lastInsertId) {
            ...
            echo "<script>window.alert('Staff Successfully Added');
            window.location.href='staff.php';        
            </script>";
            $_SESSION['message'] = "Staff Registed Successfully";
            $_SESSION['alert-class'] = "alert alert-success";
        } else {
            $_SESSION['message'] = "Sorry!! Staff Registration Failed";
